	...
	public void onEnable() {
		...
		this.getServer()
				...
				.scheduleSyncRepeatingTask(
						...
						200L,
						_thisPlugin.getHardcoreConfiguration().reaperCheckSeconds * 20L);
	...
	public void reapDeadPlayers() {
		...
		for (int i = 0; i < onlinePlayers.length; i++) {
			...
			if (this.getDeadPlayerList().isPlayerDead(playerName, true)) {
				// if so, kick them
				String livedate = this.getDeadPlayerList()
						.whenWillPlayerLive(playerName).toString();
				player.kickPlayer("You have died. You will be dead until "
						+ livedate);
				_thisPlugin.log("The reaper has caught up with " + playerName
						+ " and taken them away.");
			}
		}
